...
# CDDL HEADER END
#
#      Portions Copyright 2007-2023 Ping Identity Corporation
...
if test -z "${UNBOUNDID_SKIP_START_PRECHECK}" && test -z "${UNBOUNDID_SKIP_START_PRECHECK_NODETACH}"
then
...
then
  ...
  # This script expects an exit code of 100 to indicate the server
  # will be starting in nodetach mode, and an exit code of 99 to indicate the
  # server should be started detached.
  if test -n "${UNBOUNDID_SKIP_START_PRECHECK_NODETACH}"
  then
    EC=100
  else
    EC=99
...
$EVAL "$DRY_RUN" "$RUN" \"\${PRIVATE_UNBOUNDID_JAVA_BIN}\" \
  \${PRIVATE_UNBOUNDID_JAVA_ARGS} -Dcom.unboundid.directory.server.MaintainConfigArchive=false "${LOGGC_ARG}" \${SCRIPT_NAME_ARG} \
